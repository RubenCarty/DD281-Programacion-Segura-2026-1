...
*Tu respuesta:* ____________ segundos aproximadamente
...
**Resultado obtenido:** ___________________________________________________
...
1. Intenta el login con usuario `maria.lopez` y una contraseña incorrecta **5 veces seguidas**.
2. En el intento 5, observa qué ocurre.
3. Intenta con las credenciales correctas (`SecurePass456#`) después del bloqueo.
...
| 4 | `mal4` | |
| 5 | `mal5` | |
| 6 (con clave correcta) | `SecurePass456#` | |

**¿Cuál es el tiempo de bloqueo configurado en el sistema?** _______________
...
```bash
# Ver el log de autenticación generado
cat logs/auth.log
```
...
|---|---|---|
| `Strict-Transport-Security` | ☐ Sí ☐ No | |
| `X-Content-Type-Options` | ☐ Sí ☐ No | |
| `X-Frame-Options` | ☐ Sí ☐ No | |
| `Cache-Control` | ☐ Sí ☐ No | |
