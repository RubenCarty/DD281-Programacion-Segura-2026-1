...
**Nombre:** Javier Flores  **Código:** **Fecha:** 14/07/2026_  
...
*Justificación:* `secrets` (módulo estándar desde Python 3.6) usa `os.urandom()` que accede al CSPRNG del sistema operativo. `random` es predecible, `md5` es determinista y `uuid1` se basa en timestamp+MAC.
...
*Justificación:* Defensa en profundidad. Navegadores legacy (IE11, Safari <12, Chrome <51) no soportan SameSite. Además, ataques como el "client-side CSRF" pueden evadir SameSite.
...
*Justificación:* Las etiquetas `<img>`, `<link>`, `<iframe>` disparan peticiones GET automáticamente. Si el servidor acepta GET para acciones destructivas (violación de la semántica HTTP), el ataque prospera.
...
*Justificación:* Debería devolverse siempre el mismo código (típicamente 404) para no filtrar información sobre existencia de recursos (oráculo de información).
...
CSRF funciona porque los navegadores envían automáticamente las **cookies (de sesión)** del dominio objetivo en cada petición HTTP, incluso si la petición se originó desde un sitio diferente.

El patrón de defensa CSRF más robusto consiste en incluir un **token (CSRF)** único e impredecible en cada formulario, que el servidor verifica antes de procesar la petición.

Para almacenar contraseñas de forma segura, nunca se debe usar MD5 o SHA-1 porque son **rápidos/obsoletos/vulnerables a colisiones y rainbow tables** — en su lugar, se debe usar **bcrypt (o argon2/scrypt)**, que incorpora un salt y un factor de trabajo ajustable.

El principio de **mínimo privilegio** establece que cada usuario, proceso o componente del sistema debe tener solo los permisos estrictamente necesarios para cumplir su función.

Los secretos de una aplicación (contraseñas de BD, API keys, secret keys) nunca deben estar **hardcodeados/incrustados** en el código fuente; en su lugar deben almacenarse en **variables** de entorno o gestores de secretos.
...
| --- | --- | --- |
| 1. CSRF Token | → **c** | c) `<input type="hidden" name="csrf_token" value="{{ token }}">` |
| 2. SameSite=Strict | → **d** | d) Cookie no se envía en ningún request cross-site |
| 3. bcrypt | → **a** | a) `bcrypt.hashpw(password.encode(), bcrypt.gensalt(rounds=12))` |
| 4. Variables de entorno | → **e** | e) `SECRET_KEY = os.environ.get('SECRET_KEY')` |
| 5. Verificar propiedad (IDOR/RBAC) | → **b** | b) `SELECT * FROM datos WHERE id=? AND usuario_id=?` |
| 6. RBAC Decorador | → **f** | f) `@requiere_rol('admin', 'gerente')` sobre la función |
